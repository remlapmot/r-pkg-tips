...
- `NAMESPACE` 
  - This file is written by devtools when you run `devtools::document()` on your correctly formatted helpfiles
  ...
  - learn about environments http://adv-r.had.co.nz/Environments.html 
- Use win-builder for a further check https://win-builder.r-project.org/
- To both win-builder and CRAN you will need to submit the source version of your package as a `.tar.gz` file, e.g. in RStudio or issue `R CMD build mynewpackage` at the command line  
    ```{r}
...
- I prefer not to keep my git repositories in a folder that is also synchronized with OneDrive/Google Drive, e.g. when you render an R Markdown file the OneDrive sync of all the auxialliary files that are created can be very annoying
