...
- Don't code helpfiles (`.Rd` files) yourself 
- use Roxygen syntax through the [roxygen2 package](https://github.com/r-lib/roxygen2)
- Add the following line to your `DESCRIPTION` file
```
Roxygen: list(markdown = TRUE)
```
- Your `.R` files will look something like
```{r, eval=FALSE, echo=TRUE}
...
```
- Running `devtools::document()` will generate your `.Rd` files 
...
- Work in a new branch, especially if working with others. Then merge into main/master when you're sure your changes are what you want
- Make changes 
...
- I prefer not to keep my git repositories in a folder which is also synchronized with OneDrive/Google Drive, e.g. when you render an R Markdown file the syncing of all the auxilliary files that are created can be very annoying
